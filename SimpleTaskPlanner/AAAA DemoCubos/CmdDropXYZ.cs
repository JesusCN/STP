...
{
	class CmdDropXYZ:AsyncCommandExecuter
	{
		  STPCommandManager cmdMan;
        TaskPlanner taskplanner;

		public CmdDropXYZ(STPCommandManager cmdMan, TaskPlanner taskplanner)
            : base("dropxyz")
        {
            this.cmdMan = cmdMan;
        ...
        protected override Response AsyncTask(Command command)
        {
			...
			if (armToTake.Contains("left"))
				success = this.taskplanner.dropXYZ_LEFT(new WorldObject("unknown", new Vector3(x, y, z)));
			else if(armToTake.Contains("right"))
				success = this.taskplanner.dropXYZ_RIGHT(new WorldObject("unknown", new Vector3(x, y, z)));
			else
			{
